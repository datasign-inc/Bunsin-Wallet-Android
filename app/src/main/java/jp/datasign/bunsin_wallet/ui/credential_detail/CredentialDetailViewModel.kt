...
//)
    ...
    private val _matchedHistories = MutableLiveData<List<CredentialSharingHistory>>()
    val matchedHistories: LiveData<List<CredentialSharingHistory>> get() = _matchedHistories
    ...
    val credentialDetails: LiveData<CredentialDetails> = _credentialData.map { credential ->
        when (credential.format) {
            "vc+sd-jwt" -> {
                ...
                val claimsSorted = disclosures.sortedBy { claim ->
                    claimOrder.indexOf(claim.key).takeIf { it != -1 } ?: Int.MAX_VALUE
                }
                CredentialDetails(credential, claimsSorted, showQRCode = false)
            }
            ...
            "jwt_vc_json" -> {
                ...
                CredentialDetails(credential, claimsSorted, showQRCode = true)
            }

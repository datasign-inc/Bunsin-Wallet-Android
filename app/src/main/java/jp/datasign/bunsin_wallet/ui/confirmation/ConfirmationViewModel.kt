package jp.datasign.bunsin_wallet.ui.confirmation
...
import jp.datasign.bunsin_wallet.oid.AuthorizationServerMetadata
import jp.datasign.bunsin_wallet.utils.CredentialRequest
import jp.datasign.bunsin_wallet.utils.CredentialRequestJwtVc
import jp.datasign.bunsin_wallet.utils.CredentialRequestSdJwtVc
import jp.datasign.bunsin_wallet.utils.InvalidCredentialResponseException
import jp.datasign.bunsin_wallet.utils.KeyPairUtil
import jp.datasign.bunsin_wallet.utils.MetadataUtil
import jp.datasign.bunsin_wallet.utils.Proof
import jp.datasign.bunsin_wallet.utils.TokenErrorResponseException
import jp.datasign.bunsin_wallet.utils.UnsupportedIssuanceFlow
import jp.datasign.bunsin_wallet.utils.VCIClient
...
import jp.datasign.bunsin_wallet.vci.IssuerCredentialSubject
import jp.datasign.bunsin_wallet.vci.MetadataClient
            ...
            currentIssuerMetadata?.credentialConfigurationsSupported?.forEach { (_, credentialSupported) ->
                when (credentialSupported) {
                    is CredentialConfigurationJwtVcJson -> {
                        ...
                        val claimOrder = MetadataUtil.extractOrder(credentialSupported)
                        if (credentialSupported.credentialDefinition.type.contains(firstCredential)) {
                            val sortedClaims =
                                credentialSupported.credentialDefinition.credentialSubject?.toList()
                                    ?.sortedBy { (key, _) ->
                                        claimOrder.indexOf(key).takeIf { it != -1 } ?: Int.MAX_VALUE
                                    }
                                    ?.toMap()
                            _vct.value = firstCredential ?: ""
                            setCredentialSubject(sortedClaims ?: emptyMap())
                    ...
                    is CredentialConfigurationVcSdJwt -> {
                        ...
                        if (credentialSupported.vct == firstCredential) {
                            val sortedClaims = credentialSupported.claims.toList()
                                .sortedBy { (key, _) ->
                                    claimOrder.indexOf(key).takeIf { it != -1 } ?: Int.MAX_VALUE
                                }
                                .toMap()
                            _vct.value = firstCredential ?: ""
                            setCredentialSubject(sortedClaims)
                            updateText(currentIssuerMetadata, credentialSupported)
                        }
                    }
                }
    ...
    fun sendRequest(context: Context, txCode: String? = null) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val isKeyPairExist =
                    KeyPairUtil.isKeyPairExist(Constants.Cryptography.KEY_BINDING)
                if (isProofRequired && !isKeyPairExist) {
                    KeyPairUtil.generateSignVerifyKeyPair(Constants.Cryptography.KEY_BINDING)
                }
                ...
                if (isProofRequired) {
                    proofJwt = Proof(
                        ...
                        jwt = KeyPairUtil.createProofJwt(
                            Constants.Cryptography.KEY_BINDING,
                            credentialIssuer,

...
import jp.datasign.bunsin_wallet.utils.generateRsaKeyPair
...
//            .withHeader(x5cMap)
    ...
    suspend fun generateJwtVcCredentialData(credentialDataStore: CredentialDataStore): jp.datasign.bunsin_wallet.datastore.CredentialData? {
        val data = jp.datasign.bunsin_wallet.datastore.CredentialData.newBuilder()
    ...
    suspend fun generateSdJwtCredentialData(credentialDataStore: CredentialDataStore): jp.datasign.bunsin_wallet.datastore.CredentialData? {
    ...
    suspend fun generateHistoryData(historyStore: CredentialSharingHistoryStore): jp.datasign.bunsin_wallet.datastore.CredentialSharingHistory? {
        val claim1 = Claim.newBuilder()
            .setClaimKey("name")
            .setClaimValue("john smith")
            .setPurpose("purpose of sharing name")
            .build()
        val claim2 = Claim.newBuilder()
            .setClaimKey("location")
            .setClaimValue("tokyo")

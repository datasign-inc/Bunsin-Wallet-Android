...
import jp.datasign.bunsin_wallet.signature.SignatureUtil.derToRaw
import jp.datasign.bunsin_wallet.signature.SignatureUtil.validateCertificateChain
import jp.datasign.bunsin_wallet.utils.KeyPairUtil
import com.fasterxml.jackson.module.kotlin.jacksonObjectMapper
import jp.datasign.bunsin_wallet.Constants
import jp.datasign.bunsin_wallet.signature.SignatureUtil.convertPemToX509Certificates
...
class JWT {
    companion object {
        ...
        fun sign(keyAlias: String, header: Map<String, Any>, payload: Map<String, Any>): String {
            ...
            val signature: ByteArray = Signature.getInstance(Constants.Cryptography.SIGNING_ALGORITHM).run {
                initSign(privateKey)
                update(dataBytes)
                sign()
            }
